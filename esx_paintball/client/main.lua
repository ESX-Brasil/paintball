...
Citizen.CreateThread(function()
	...
	while true do
		...
		DrawMarker(1,tenue.x,tenue.y,tenue.z-1,0,0,0,0,0,0,2.001,2.0001,0.5001,0,155,255,200,0,0,0,0) -- Guarda-roupa para paintball
		...
		if(isNear(tenue)) then
				Info("Pressionado ~g~E~w~ para mudá-lo.")
		...
		if(isNear(gamestart)) then
				Info("Pressionado ~g~E~w~ para começar o jogo.")
				if(IsControlJustPressed(1, 38)) then
					debutpartie()
				end
		end
		...
		if(isNear(miseazero)) then
				Info("Pressionado ~g~E~w~ para redefinir a pontuação.")
				if(IsControlJustPressed(1, 38)) then
					remettrezero()
				end
		end
		...
		if (Vdist(pos.x, pos.y, pos.z, home.x, home.y, home.z) < 2.0) then
				Info("Pressionado ~g~E~w~ para jogar paintball.")
				if(IsControlJustPressed(1, 38)) then
					allerjouer()
				end
		end
		...
		if (Vdist(pos.x, pos.y, pos.z, activity.x, activity.y, activity.z) < 2.0) then
				Info("Pressionado ~g~E~w~ para sair do paintball.")
				if(IsControlJustPressed(1, 38)) then
					quitterpaintball()
				end
		end
...
function OuvrirVestiaire()
  ...
  local elements = {
    ...
    table.insert(elements, {label = "Equipe roupa 1", value = 'tenue_equipe'})
	table.insert(elements, {label = "Equipe roupa 2", value = 'tenue_equipedeux'})
...
function debutpartie()
	...
	if(distancenotification(millieu)) then
		...
		ESX.ShowNotification('~p~O jogo começa em ~r~~h~3')
		Citizen.Wait(1000)
		ESX.ShowNotification('~p~O jogo começa em ~r~~h~2')
		Citizen.Wait(1000)
		ESX.ShowNotification('~p~O jogo começa em ~r~~h~1')
		Citizen.Wait(1000)
		ESX.ShowNotification('~p~A partida ~r~~h~começou !')
	end
end
...
Citizen.CreateThread(function()
    function Initialize(scaleform)
        ...
        PushScaleformMovieFunctionParameterString("~r~Bom paintball!!")
		PushScaleformMovieFunctionParameterString("Espero que suas armas estejam no seu baú!")
...
ESX.ShowNotification('~p~A pontuação está agora em zero')
end
